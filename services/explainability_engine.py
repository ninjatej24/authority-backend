...
def _validation(
    *,
    ...
    if not evidence_ids and not moment_ids and claim_id not in {"authority_score", "dimension_scores", "scenario_adjustments", "progress_interpretation"}:
...
def build_explainability(
    *,
    ...
    del metrics
    valid_evidence_ids = {item.id for item in evidence} | {item.evidence_id for item in report.evidence_chain} | {
        item.evidence_id for item in psychological_inference.evidence_chain
    }
    valid_moment_ids = {item.moment_id for item in moments} | {item.moment_id for item in report.timeline}
    ...
    orphan_evidence = sorted({e for claim in claims for e in claim.supporting_evidence_ids if e not in valid_evidence_ids})
    orphan_moments = sorted({m for claim in claims for m in claim.supporting_moments if m not in valid_moment_ids})
    valid_count = len(claims) - len(failed)
    total = max(len(claims), 1)
    integrity = round(max(0.0, 1.0 - (len(failed) + len(orphan_evidence) + len(orphan_moments)) / total), 2)
